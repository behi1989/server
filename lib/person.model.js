        ...
        if (!res)
          ...
          if (isAgent) {
            ...
            if (user.hasOwnProperty('warehouse_id')) {
              ...
              // socket.setRoom(user.warehouse_id);
    ...
    person.loginCheck(username, password, personType, req.body.loginType)
      .then(foundPerson => {
        ...
        if (req.body.loginType) {
          if (['Clerk', 'SalesManager', 'DeliveryAgent'].map(r => _const.ACCESS_LEVEL[r].toString()).includes(req.body.loginType.toString())) {
            ...
            warehouse.getAllWarehouses().then(warehouses => {
              ...
              if (req.body.loginType.toString() === _const.ACCESS_LEVEL.Clerk.toString() &&
                req.body.warehouse_id === warehouses.find(x => x.is_center)._id.toString()) {
