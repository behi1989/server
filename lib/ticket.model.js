...
class Ticket {
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'scanReady') {
      ...
      search = this.searchScanReadyItems(user.warehouse_id, offset, limit, user.access_level);
    }
  ...
  searchInbox(receiverId, offset, limit, access_level) {
    let agentMatchOption;
    let matchForSalesmanager = [
      {
    ...
    if (access_level !== _const.ACCESS_LEVEL.SalesManager)
      agentMatchOption = matchForOthers;
    else
      agentMatchOption = matchForSalesmanager;
    const
      result = {
        mainQuery: [],
        countQuery: []
  ...
  searchScanReadyItems(receiverId, offset, limit, access_level) {
